...
import { str, flag } from './config.js';
...
export function identityDefault() {
  return flag('EMBER_IDENTITY', 'identity', true);
}

function readOne(kind) {
  const explicit = str(`EMBER_${kind}_PATH`, kind === 'SOUL' ? 'soulPath' : 'userPath');
  const dir = str('EMBER_IDENTITY_DIR', 'identityDir', DEFAULT_DIR);
...
export function loadAgentsMd(workspace) {
  const candidates = [
    ...
    str('EMBER_AGENTS_PATH', 'agentsPath'),
...
export function loadCraft() {
  if (!flag('EMBER_CRAFT', 'craft', false)) return { text: '', source: null };
  const p = str('EMBER_CRAFT_PATH', 'craftPath', path.join(HERE, '..', 'CRAFT.md'));
  try {
