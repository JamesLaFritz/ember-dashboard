...
export class LMStudio {
  ...
  async models() {
    ...
    return (data.models ?? []).map(m => {
      ...
      // LM Studio exposes TWO strings for one loaded model and accepts either
      // for inference: `key` (with the quant suffix) and the loaded instance's
      // `id` (without it) — the one `lms ps` prints, so the one a human copies.
      // Callers that resolve a session's model must try both or they silently
      // miss; see #ctxWindow().
      instanceIds: (m.loaded_instances ?? []).map(i => i.id).filter(Boolean),
  ...
  // `maxTokens` is per request, not the constant: a context window is shared by
  // prompt and completion, so the safe generation cap is min(M, C − I − S) and
  // the binding term changes as the conversation grows.
  async chatStream({ model, messages, tools, temperature = 0.3, onDelta, onReasoning,
    ...
    try {
      res = await fetch(this.base + '/v1/chat/completions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ model, messages, tools, temperature, stream: true, max_tokens: maxTokens, stream_options: { include_usage: true } }),
        signal: ac.signal,
      });
    } catch (err) {
      clearTimeout(idle);
