...
export const MAX_OUTPUT_TOKENS = Number(process.env.EMBER_MAX_TOKENS) || 16384;
...
export const STREAM_IDLE_MS = Number(process.env.EMBER_STREAM_IDLE_MS) || 120000;
...
export class LMStudio {
  ...
  async chatStream({ model, messages, tools, temperature = 0.3, onDelta, onReasoning }) {
    ...
    const kick = () => { clearTimeout(idle); idle = setTimeout(() => ac.abort(), STREAM_IDLE_MS); };
    ...
    for await (const chunk of res.body) {
      ...
      if (ac.signal.aborted) {
        throw new Error(`LM Studio sent nothing for ${STREAM_IDLE_MS / 1000}s — treating the stream as stalled. `
