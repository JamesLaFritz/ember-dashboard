...
export class SkillRunner {
  constructor({ claudeCommand, vaultPath, broadcast, allowedTools = [] }) {
  ...
  run(skillId, args = '', model) {
    const id = randomUUID().slice(0, 8);
    const prompt = `/${skillId} ${args}`.trim();
    // Headless runs are non-interactive — nothing can grant permission
    // mid-flight, so gated tools must be pre-allowed here or the skill dies
    // (trend-watch's web access, most skills' vault writes). acceptEdits
    // covers file edits; extra tools come from config.claudeHeadlessAllow.
