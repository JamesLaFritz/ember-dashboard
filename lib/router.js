...
import { identityPreamble } from './identity.js';
...
export class Router {
  ...
  async localAgentAnswer(text) {
    ...
    const messages = [
      { role: 'system', content:
`${identityPreamble('You are Ember, the resident intelligence of this agentic OS — an Obsidian vault the operator works out of.')}
...
Snapshot: ${vitals.project.label ?? "project"} BRIEF ${vitals.project.brief?.done}/${vitals.project.brief?.total}; ${vitals.vault.wikiPages} wiki pages; last session ${vitals.vault.lastSessionName}.` },
  ...
  async rundown() {
    ...
    if (vitals.project.configured) {
      pieces.push(`${vitals.project.label ?? 'Project'} brief: ${vitals.project.brief?.done ?? 0}/${vitals.project.brief?.total ?? 0} done, ${vitals.project.openLoops} open loops, ` +
        (vitals.project.daysSinceDevlog == null ? 'no devlog yet.' : `last devlog ${vitals.project.daysSinceDevlog}d ago.`));
    }
    ...
    try {
      reply = await this.lm.complete({
        ...
        system: identityPreamble('You are Ember, the operator\'s direct, concise copilot.')
          + '\nTurn the following status notes into a spoken-style rundown of at most 6 sentences. Lead with whatever most moves the operator\'s current project forward. No greetings longer than three words, no bullet points, no sycophancy.',
        user: pieces.join('\n\n'),
        maxTokens: 4096,
      });
    } catch {
