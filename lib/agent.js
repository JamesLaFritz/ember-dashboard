  ...
  async send(userText) {
    ...
    try {
      ...
      for (let hop = 0; hop < MAX_HOPS; hop++) {
        ...
        // draws, and just as invisible if left unreported.
        if (finishReason === 'length') {
          const note = `

[truncated at the ${MAX_OUTPUT_TOKENS}-token output ceiling — this reply is incomplete. Continue in a new message, or raise EMBER_MAX_TOKENS.]`;
          this.messages.push({ role: 'assistant', content: stripThink(content) + note });
          this.#record({ kind: 'assistant', text: content + note });
