...
import { composeSystem, loadIdentity, loadAgentsMd, loadCraft } from './identity.js';
...
export const MAX_HOPS = Number(process.env.EMBER_MAX_HOPS) || 120;
...
export const MAX_AUTO_CONTINUE = Number(process.env.EMBER_MAX_AUTO_CONTINUE ?? 3);
...
export class AgentSession {
  constructor({ id, lm, mcp, model, workspace, preset, mode, broadcast, onDirty, stateDir }) {
    this.id = id;
    this.lm = lm;
    ...
    this.messages = [{ role: 'system', content: systemFor(preset, workspace, this.stateDir) }];
  ...
  async report() {
    ...
    const cachedCtx = this.contextWindow;
    const ctx = cachedCtx ?? await this.#ctxWindow();
    const ctxNote = ctx == null ? '' : cachedCtx ? '' : ' _(read now, not during the run — verify the model was not reloaded since)_';
    ...
    const idSources = loadIdentity().sources;
    const idOn = Object.values(idSources).some(Boolean);
    ...
    const rows = [
      ...
      ['Loaded context length', ctx ? `**${ctx.toLocaleString()}**${ctxNote}` : '_(unknown — LM Studio did not report a loaded instance for this model)_'],
      ...
      ['**Identity layer**', idOn
        ? `⚠️ **ON** — ${Object.entries(idSources).filter(([, v]) => v).map(([k, v]) => `${k}: \`${v}\``).join(' · ')}`
        : 'off (`EMBER_IDENTITY=off`) — correct for a comparison run'],
...
[truncated at the ${MAX_OUTPUT_TOKENS}-token output ceiling ${MAX_AUTO_CONTINUE + 1} times in a row — stopping. This reply is incomplete. Send "continue", raise EMBER_MAX_TOKENS, or raise EMBER_MAX_AUTO_CONTINUE.]`;
  ...
  async compact(reason = 'manual') {
    ...
    this.#writeSessionLog({ reason, before, after, folded: userIdxs.length - this.keepRecentTurns, summary: summary.trim() });
    this.onDirty(this);
    return true;
  }
...
export class AgentManager {
  ...
  clear(id) {
    ...
    s.messages = [{ role: 'system', content: systemFor(s.preset, s.workspace, s.stateDir) }];
    s.history = [];
  ...
  setPreset(id, preset) {
    ...
    s.preset = preset;
    s.messages[0] = { role: 'system', content: systemFor(preset, s.workspace, s.stateDir) };
    this.#persist(s);
    return preset;
  }
