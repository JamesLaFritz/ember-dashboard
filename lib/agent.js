...
export class AgentSession {
  ...
  report() {
    ...
    const rows = [
      ...
      ['Loaded context length', ctx ? `**${ctx.toLocaleString()}**` : '_(not read — no turn has completed yet)_'],
      ...
      ['Identity files', Object.entries(loadIdentity().sources).map(([k, v]) => `${k}: ${v ?? 'none'}`).join(' · ') || 'none'],
