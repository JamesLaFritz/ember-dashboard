...
export const PRESETS = {
  'coding-agent': {
    label: 'Coding Agent',
    description: 'Unity/C#-fluent implementer. Reads before editing, minimal diffs, verifies its own work before reporting done, terse reports. Writes and commands always gated on your approval.',
...
export class AgentSession {
  ...
  toJSON() {
    return {
      ...
      shell: SHELL_NAME, shellPath: SHELL_PATH,
    };
  }
  ...
  async compact(reason = 'manual') {
    ...
    this.#record({ kind: 'compacted', reason, folded: userIdxs.length - this.keepRecentTurns });
      ...
      switch (tc.name) {
        ...
        case 'run_command': {
          return await new Promise((resolve) => {
            const child = spawn(args.command, { cwd: this.workspace, shell: SHELL_PATH, timeout: 120000 });
            let out = '';
