...
export class AgentSession {
  ...
  async report() {
    ...
    // ANY bail, not just the two that had rows. The harness emits four reasons —
    // output_ceiling, max_hops, reasoning_overrun, empty_turn — and the report
    // only ever asked about the first two, so a run stopped by either of the
    // others was printed as "Harness bail: no". Measured 2026-09-04: a 197-hop
    // run ended on an empty_turn bail and the report denied it happened. Match on
    // the kind and read the reason off the record, so a reason added later cannot
    // go silent by omission.
    const anyBail = this.history.find(h => h.kind === 'bail');
    ...
    const outcome = [
      ...
      // An empty turn is a HARNESS recovery (Fix 15): the model returned nothing
      // usable and the harness re-asked. Unreported, the retry reads as the model
      // hesitating, and an exhausted retry budget reads as the model quitting.
      // Both are misattributions a score would silently inherit.
      ['**Empty turns retried**', n('empty_turn')
        ? `${n('empty_turn')}${n('empty_turn', h => h.strayCall) ? ' — at least one carried a stray tool call' : ''}`
        : '0'],
      ...
      ['**Harness bail**', hopBail ? `**yes — hop ceiling (${MAX_HOPS})**, do not score axis 8`
        : ceilingBail ? '**yes — output ceiling**, reply incomplete'
        : anyBail ? `**yes — ${String(anyBail.reason ?? 'unknown').replace(/_/g, ' ')}**, the harness stopped the turn — do not score it as finished`
        : 'no'],
    ];
    ...
    return [
      ...
      compactions.length
        ? ['| # | At turn | Tokens before | Tokens after | Turns folded | Window | Summary chars | Results evicted |', '|---|---|---|---|---|---|---|---|',
           ...compactions.map((c, i) => `| ${i + 1} | ${c.atTurn ?? '—'} | ${c.before ?? '—'} | ${c.after ?? '—'} | ${c.folded ?? '—'} | ${c.window ?? '—'} | ${c.summaryChars ?? '—'} | ${c.evictedResults ? `**${c.evictedResults}** (${(c.evictedChars ?? 0).toLocaleString()} chars)` : '0'} |`)].join('\n')
...
[HARNESS: you emitted ${toolCalls.length} tool calls in this turn. `
  ...
  async compact(reason = 'manual') {
    ...
    // Two numbers, deliberately. `before` is the SERVER's tokenizer count and is
    // what the run record should carry. `beforeEst` is the same crude ~4 chars/token
    // estimator used for `after`, and exists solely so the before→after delta is
    // like-for-like. Comparing a real token count against an estimate made one fold
    // read as 90,829 → 91,544 — a compaction that appeared to GROW the context.
    const before = this.lastPromptTokens || Math.round(JSON.stringify(this.messages).length / 4);
    const beforeEst = Math.round(JSON.stringify(this.messages).length / 4);
    ...
    // Keep the pre-fold history so a fold that achieves nothing can be undone.
    // Eviction is PART of what a fold accomplishes — on a single oversized group
    // it is the only part — so the size test has to come after it, which means
    // mutating first and rolling back rather than projecting.
    ...
    // A fold costs a summariser call and trades real history for a lossy note.
    // If the result is not smaller it bought nothing, cost context, and the next
    // check fires again immediately — measured 2026-09-04, a fold recorded as
    // 90,829 → 91,544 followed by a second fold six turns later. Roll back and
    // say so rather than pay twice.
    if (after >= beforeEst) {
      this.messages = snapshot;
      const text = `Compaction ABORTED: folding ${foldedUsers} user turn(s) would leave the context at `
        + `~${after.toLocaleString()} tokens against ~${beforeEst.toLocaleString()} before — the ${summary.trim().length}-char `
        + `summary costs more than the folded turns saved. History restored; the cut point needs to reach further back.`;
      this.#record({ kind: 'compact_failed', reason, text, atTurn: this.stats.turns, before, beforeEst, after });
      this.broadcast({ type: 'agent_tool', session: this.id, tool: 'compact', args: 'aborted: fold would not shrink' });
      console.error(`agent: ${text} (session ${this.id})`);
      this.onDirty(this);
      return false;
    }
    ...
    this.#record({
      ...
      folded: foldedUsers,
      before, beforeEst, after,                   // `before` is the server count; beforeEst/after are the same estimator, so their delta is real
